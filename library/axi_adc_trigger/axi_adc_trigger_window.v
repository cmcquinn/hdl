...
module axi_adc_trigger_window #(
  ...
  input       [31:0]    window_limit,
  input       [31:0]    limit_interval, // hysteresis

  output reg  [31:0]    read_window_cnt,
  ...
  wire                  passthrough_low_l1_s;  // trigger when fallingh thorugh the limit
  wire                  passthrough_high_l2_s; // trigger when rising through the limit
  wire                  passthrough_low_l2_s;  // trigger when fallingh thorugh the limit
  ...
  reg        [15:0]     stop_cnt;
  reg        [31:0]     window_cnt;
  reg        [31:0]     window_cnt_m;

  reg                   start_window_cnt;
  reg                   stop_window_cnt;
  reg                   run_window_cnt;
  reg        [31:0]     first_window_limit;
  reg        [31:0]     second_window_limit;
  reg                   window_cnt_pass_first;
  reg                   window_cnt_pass_second;
  reg                   auto_reset;
  ...
  assign data_l2_cmp   = mux_sel_l2_data ? data_a[DW:0] : data_b[DW:0];
  ...
  // N start stop delay counters

  assign start_cnt_in = trigger_l1;
  assign stop_cnt_in = trigger_l2;

  // start
  always @(posedge clk) begin
    if ((start_cnt == 15'd0) & (!run_window_cnt | rst_at_new_start_pulse)) begin
      start_cnt <= cnt_start;
      start_window_cnt <= start_cnt_in;
    end else if (start_cnt_in) begin
    ...
    // stop
    if ((stop_cnt == 15'd0) & run_window_cnt) begin
      stop_cnt <= cnt_stop;
      stop_window_cnt <= stop_cnt_in;
    end else if (stop_cnt_in) begin
  ...
  // limit conunter interval
  always @(posedge clk) begin
    first_window_limit <= window_limit - limit_interval;
    second_window_limit <= window_limit - limit_interval;
  end
  ...
  always @(posedge clk) begin
    ...
    read_window_cnt <= window_cnt_m;

    // run window counter
    if(run_window_cnt) begin
      window_cnt <= window_cnt + 1;
    end

    // stop window counter
    if ((window_cnt > first_window_limit) & first_window_limit != 0) begin
      window_cnt_pass_first <= 1'b1;
    end
    if ((window_cnt > second_window_limit) & second_window_limit != 0) begin
